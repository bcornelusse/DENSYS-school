...
\section[Introduction and Categories \\ of \\ Machine Learning problems]{Introduction}
\begin{frame}{Machine Learning}
  
  Machine learning is the field of computer algorithms that improve automatically through experience and data.
  Common applications:
      \begin{itemize}
        \item Large Language Models / natural language processing
...
\begin{column}{0.5\textwidth}
  \begin{center}
    %TODO add a view where clusters are not present
...
Example application: 
\begin{itemize}
  \item you have 365 days of consumption data with 15 minute resolution, 
  \item identify e.g. a subset of 12 days that represent well the 365 days.
  \item useful to reduce the size of a microgrid sizing problem
\end{itemize}
...
\begin{frame}[allowframebreaks]{Reinforcement Learning}
  ...
  \begin{itemize}
    ...
    \item Goal: learn to take actions ($a$) in any given state ($s$) to maximize a rewards collected.
  \end{itemize}
\end{column}
\begin{column}{0.45\textwidth}

  \begin{center}
    \includegraphics[width=\textwidth]{images/RL.png}
  \end{center}
\end{column}
\end{columns}

\begin{block}{Maze example}
Consider an agent 
\begin{itemize}
  \item who wants to exit of a maze of dimensions $m$ and $n$ 
  \item and can go left, right, up or down.  
\end{itemize}
\end{block}

Questions:
\begin{itemize}
  \item What is the size of the state space?
  \item What is the size of the action space?
  \item How would you \textit{shape} the rewards to incentivize the robot getting out as fast as possible (least number of steps, and avoid bumping into walls)?
\end{itemize}

One way to solve this problem is to compute a state-value function $Q(s,a)$ that expresses 
the expected cumulative reward (or return) when starting from state $s$ and taking action $a$, following an optimal policy thereafter.

\begin{center}
\href{https://colab.research.google.com/drive/1SoSubqykgjXIe6-UMHTZgH9xfg6dQ4xG?usp=sharing}{\alert{\underline{Link to the colab example}}}  
\end{center}
...
\begin{frame}{Supervised Learning (SL)}
  \begin{itemize}
    \item Data organized as a set of $N$ \textbf{objects} described by input features and output labels.
    $$\{(x_1, y_1), (x_2, y_2), \dots, (x_N, y_N)\} $$
    \item \textbf{Features} $x_i$ are easily obtained by observation.
    \item \textbf{Output label} $y_i$ is provided by an expert or is the result of difficult/costly observation/computation.
  \end{itemize}
\end{frame}
...
\begin{frame}{Empirical Loss Minimization}
  \begin{itemize}
    \item We usually don't know the joint distribution over inputs and outputs $\mathbb{P}_{X,Y}$.
    \item Instead, we thus minimize an estimate of the expected loss, the \textbf{empirical loss}:
...
\begin{frame}{Linear Model}
  % todo apply linear regression to actual data such as price of electricity as a function of time of the day on data from belgium in 2025
  \begin{columns}
    \begin{column}{0.5\textwidth}
      \begin{itemize}
...
\begin{frame}{Polynomial of degree 4}
  % todo apply polynomial regression deg 4 to actual data such as price of electricity as a function of time of the day on data from belgium in 2025
  \begin{columns}
    \begin{column}{0.5\textwidth}
  \begin{itemize}
...
\begin{frame}{Polynomial of degree 20}
  % todo apply polynomial regression deg 20 to actual data such as price of electricity as a function of time of the day on data from belgium in 2025
  \begin{columns}
    \begin{column}{0.5\textwidth}
  \begin{itemize}
...
\begin{frame} {Model Selection}
  % todo compute actual train and test values an report in a table
  \begin{columns}
    \begin{column}{0.5\textwidth}
