...
\begin{frame}[allowframebreaks]{To go further}

If you have completed the model and it runs: 
\begin{itemize}
	\item Analyze the flows and the total cost
	\item Are there some "non-intuitive" flows? How could you avoid them?
\end{itemize}

\textbf{Sensitivity analysis.} For all the cases below, think first about the expected solution and check the solver results. Is the problem infeasible/unbounded for some values? How does the cost function evolve? 
\begin{itemize}
	\item Change the costs of the generator and the PCC
	\item Change the capacity of the lines, either uniformly with the SCALE{\_}RATING parameter, or line by line. 
	\item Change the loads 
\end{itemize}

This model is not "limited to a microgrid". It could be used to approximate/represent any energy network. Modify the network structure to model e.g. a part of the European netwok, where a node represents a country
\begin{itemize}
	\item Add nodes and lines
	\item Add generators and loads
	\item Do you need a PCC? If so, where would you place it?
	\item How would you get the price of electricty in a country?
\end{itemize}
\end{frame}
